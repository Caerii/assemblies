#!/usr/bin/env python3
"""
Working Purkinje Cell Brain Simulation
...
class WorkingPurkinjeBrain:
    ...
    def __init__(self, timestep_ms=0.1, seed=42):
        """Initialize working Purkinje brain simulation"""
        ...
        self.populations = {
            ...
            'purkinje': {
                ...
                'rate_std': 10.0,
                # WORKING: Purkinje cells have intrinsic firing but with proper constraints
                ...
                'active_fraction_100ms': 0.2,  # 20% per 100ms (realistic with refractory)
    ...
    def simulate_realistic_step(self):
        ...
        for name, pop in self.population_data.items():
            ...
            # For Purkinje cells, also check complex spike suppression
            if name == 'purkinje':
            ...
            if not cp.any(non_refractory):
                continue  # All neurons are refractory or suppressed
            ...
            if cp.any(spike_mask):
                ...
                # For Purkinje cells, occasionally generate complex spikes
                if name == 'purkinje':
                    # 1% chance of complex spike per simple spike
                    complex_spike_mask = cp.random.random(len(winners)) < 0.01
                    if cp.any(complex_spike_mask):
                        complex_winners = winners[complex_spike_mask]
    ...
    def simulate_detailed(self, n_steps=100):
        ...
        print(f"\n🧠 Working Purkinje Simulation ({n_steps} steps, {self.timestep_ms}ms timestep):")
...
def test_working_purkinje_simulation():
    """Test working Purkinje brain simulation with different timesteps"""
    print("🧠 WORKING PURKINJE BRAIN SIMULATION")
    print("=" * 80)
    ...
    for timestep in timesteps:
        ...
        try:
            sim = WorkingPurkinjeBrain(timestep_ms=timestep, seed=42)
...
if __name__ == "__main__":
    # Test working Purkinje simulation
    test_working_purkinje_simulation()
