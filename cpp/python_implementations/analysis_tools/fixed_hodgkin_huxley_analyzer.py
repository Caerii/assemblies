#!/usr/bin/env python3
"""
Fixed Hodgkin-Huxley Analyzer
Proper stimulation currents and voltage monitoring for realistic firing
"""
...
class FixedHodgkinHuxleyNeuron:
    """
    Fixed Hodgkin-Huxley neuron with proper stimulation and voltage monitoring
    """
    
    def __init__(self, n_neurons, dt_ms=0.01):
        """Initialize fixed Hodgkin-Huxley neurons"""
        ...
        # Spike detection
        self.spike_threshold = -55.0  # Lower threshold for easier firing
        ...
        self.max_history = 1000  # Keep last 1000 voltage values
    ...
    def step(self, I_inj=None, synaptic_input=None):
        """Integrate one timestep with voltage monitoring"""
        ...
        # Ensure shapes match
        dt_tau_m = self.dt_s / tau_m
        dt_tau_h = self.dt_s / tau_h
        dt_tau_n = self.dt_s / tau_n
        
        self.m += (m_inf - self.m) * dt_tau_m
        self.h += (h_inf - self.h) * dt_tau_h
        self.n += (n_inf - self.n) * dt_tau_n
...
class FixedHodgkinHuxleyAnalyzer:
    """
    Fixed Hodgkin-Huxley Analyzer with proper stimulation and monitoring
    """
    
    def __init__(self, n_neurons=10000, seed=42):
        """Initialize fixed Hodgkin-Huxley analyzer"""
        self.n_neurons = n_neurons
        self.seed = seed
    ...
    def test_timestep_with_proper_stimulation(self, dt_ms, n_steps=1000, stimulation_type='constant'):
        ...
        hh_neurons = FixedHodgkinHuxleyNeuron(self.n_neurons, dt_ms)
        
        # Get initial memory
        initial_cpu_memory = get_memory_usage()
        initial_gpu_memory = analyze_gpu_memory()
        
        # Generate proper input current based on stimulation type
        if stimulation_type == 'constant':
            I_inj = cp.full(self.n_neurons, 20.0, dtype=cp.float32)  # Higher current
        elif stimulation_type == 'random':
            I_inj = cp.random.normal(20.0, 10.0, self.n_neurons).astype(cp.float32)  # Higher random current
            ...
            I_inj = cp.full(self.n_neurons, 20.0, dtype=cp.float32)
        ...
        for step in range(n_steps):
            ...
            if stimulation_type == 'pulse':
                # Pulse every 100 steps with higher current
                if step % 100 < 50:
                    I_inj = cp.full(self.n_neurons, 30.0, dtype=cp.float32)
                else:
                    I_inj = cp.zeros(self.n_neurons, dtype=cp.float32)
            elif stimulation_type == 'sinusoidal':
                # Sinusoidal input with higher amplitude
                t = step * dt_ms / 1000.0
                I_inj = 20.0 + 15.0 * cp.sin(2 * cp.pi * 10 * t)  # 10 Hz oscillation
    ...
    def test_ultra_fine_timesteps_fixed(self):
        """Test ultra fine timesteps with fixed stimulation"""
        print("🔬 ULTRA FINE TIMESTEP ANALYSIS (FIXED)")
        print("=" * 80)
        
        # Test timesteps with proper stimulation
        ...
        for stimulation_type in stimulation_types:
            print(f"\n{'='*80}")
            print(f"🧪 TESTING: {stimulation_type.upper()} STIMULATION (FIXED)")
            print(f"{'='*80}")
            ...
            for dt_ms in timesteps:
                ...
                try:
                    result = self.test_timestep_with_proper_stimulation(dt_ms, n_steps=1000, stimulation_type=stimulation_type)
            ...
            print(f"\n📊 {stimulation_type.upper()} STIMULATION SUMMARY (FIXED):")
...
if __name__ == "__main__":
    # Test fixed Hodgkin-Huxley
    analyzer = FixedHodgkinHuxleyAnalyzer(n_neurons=10000, seed=42)
    
    # Test voltage monitoring
    analyzer.test_voltage_monitoring()
    
    # Test ultra fine timesteps with fixed stimulation
    results = analyzer.test_ultra_fine_timesteps_fixed()
