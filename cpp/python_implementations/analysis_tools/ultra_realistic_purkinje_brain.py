#!/usr/bin/env python3
"""
Ultra Realistic Purkinje Cell Brain Simulation
Properly models Purkinje cell dynamics with realistic activation patterns
"""
...
class UltraRealisticPurkinjeBrain:
    """
    Ultra Realistic Purkinje Cell Brain Simulation
    Properly models Purkinje cell dynamics with realistic activation patterns
    """
    ...
    def __init__(self, timestep_ms=0.1, seed=42):
        """Initialize ultra realistic Purkinje brain simulation"""
        ...
        self.populations = {
            ...
            'purkinje': {
                ...
                'rate_std': 10.0,
                # ULTRA REALISTIC: Purkinje cells have intrinsic firing but with proper constraints
                'active_fraction_1ms': 0.05,  # 5% per ms (realistic with refractory)
                'active_fraction_10ms': 0.4,  # 40% per 10ms (realistic with refractory)
                'active_fraction_100ms': 0.3,  # 30% per 100ms (realistic with refractory)
                'refractory_period_ms': 2.0,  # 2ms refractory period
                'complex_spike_suppression_ms': 20.0,  # 20ms suppression after complex spike
                'burst_probability': 0.1,  # 10% chance of burst firing
                'burst_duration_ms': 5.0,  # 5ms burst duration
            }
        }
        ...
        print(f"🧠 Ultra Realistic Purkinje Brain Simulation")
    ...
    def _initialize_simulation(self):
        """Initialize the ultra realistic simulation"""
        print(f"\n🚀 Initializing Ultra Realistic Purkinje Brain:")
        ...
        for name, pop in self.populations.items():
            ...
            self.population_data[name] = {
                ...
                'refractory_period': cp.full(k_active, pop.get('refractory_period_ms', 2.0), dtype=cp.float32),
                'complex_spike_time': cp.zeros(k_active, dtype=cp.float32),  # For Purkinje cells
                'complex_spike_suppression': cp.full(k_active, pop.get('complex_spike_suppression_ms', 0.0), dtype=cp.float32),
                'burst_start_time': cp.zeros(k_active, dtype=cp.float32),  # For Purkinje cells
                'burst_duration': cp.full(k_active, pop.get('burst_duration_ms', 0.0), dtype=cp.float32),
                'activated': False
            }
        ...
        max_active = max(pop['k_active'] for pop in self.population_data.values())
        self.candidates = cp.zeros(max_active * 10, dtype=cp.float32)
        self.top_k_indices = cp.zeros(max_active, dtype=cp.int32)
        self.top_k_values = cp.zeros(max_active, dtype=cp.float32)
        self.sorted_indices = cp.zeros(max_active, dtype=cp.int32)
    ...
    def simulate_realistic_step(self):
        """Simulate one realistic step with proper Purkinje cell dynamics"""
        ...
        for name, pop in self.population_data.items():
            ...
            # For Purkinje cells, also check complex spike suppression and burst dynamics
            if name == 'purkinje':
                non_suppressed = (current_time - pop['complex_spike_time']) > pop['complex_spike_suppression']
                non_refractory = non_refractory & non_suppressed
                
                # Check if in burst mode
                in_burst = (current_time - pop['burst_start_time']) < pop['burst_duration']
                
                # If in burst, increase firing rate
                if cp.any(in_burst):
                    burst_mask = in_burst & non_refractory
                    if cp.any(burst_mask):
                        # Burst firing: 5x normal rate
                        burst_firing_rates = pop['firing_rates'][burst_mask] * 5.0
                        burst_probabilities = burst_firing_rates * (self.timestep_ms / 1000.0)
                        burst_probabilities = cp.minimum(burst_probabilities, 1.0)
                        
                    burst_random_values = cp.random.random(int(cp.sum(burst_mask)))
                    burst_spike_mask = burst_random_values < burst_probabilities
                    
                    if cp.any(burst_spike_mask):
                        burst_indices = cp.where(burst_mask)[0]
                        burst_winners = burst_indices[burst_spike_mask]
                        
                        # Update burst winners
                        pop['w'] = len(burst_winners)
                        pop['winners'][:len(burst_winners)] = burst_winners
                        pop['activated'] = True
                        pop['weights'][burst_winners] += 0.1
                        pop['weights'] *= 0.99
                        pop['support'][burst_winners] += 1.0
                        pop['last_spike_time'][burst_winners] = current_time
                        
                        # Occasionally generate complex spikes during bursts
                        complex_spike_mask = cp.random.random(len(burst_winners)) < 0.05  # 5% chance
                        if cp.any(complex_spike_mask):
                            complex_winners = burst_winners[complex_spike_mask]
                            pop['complex_spike_time'][complex_winners] = current_time
                
                # Normal firing for non-burst neurons
                normal_mask = ~in_burst & non_refractory
                if cp.any(normal_mask):
                    normal_firing_rates = pop['firing_rates'][normal_mask]
                    normal_probabilities = normal_firing_rates * (self.timestep_ms / 1000.0)
                    normal_probabilities = cp.minimum(normal_probabilities, 1.0)
                    
                    normal_random_values = cp.random.random(int(cp.sum(normal_mask)))
                    normal_spike_mask = normal_random_values < normal_probabilities
                    
                    if cp.any(normal_spike_mask):
                        normal_indices = cp.where(normal_mask)[0]
                        normal_winners = normal_indices[normal_spike_mask]
                        
                        # Update normal winners
                        pop['w'] = len(normal_winners)
                        pop['winners'][:len(normal_winners)] = normal_winners
                        pop['activated'] = True
                        pop['weights'][normal_winners] += 0.1
                        pop['weights'] *= 0.99
                        pop['support'][normal_winners] += 1.0
                        pop['last_spike_time'][normal_winners] = current_time
                        
                        # Occasionally start bursts
                        burst_start_mask = cp.random.random(len(normal_winners)) < 0.1  # 10% chance
                        if cp.any(burst_start_mask):
                            burst_start_winners = normal_winners[burst_start_mask]
                            pop['burst_start_time'][burst_start_winners] = current_time
                        
                        # Occasionally generate complex spikes
                        complex_spike_mask = cp.random.random(len(normal_winners)) < 0.01  # 1% chance
                        if cp.any(complex_spike_mask):
                            complex_winners = normal_winners[complex_spike_mask]
                            pop['complex_spike_time'][complex_winners] = current_time
            else:
                # Normal firing for other populations
                if cp.any(non_refractory):
                    spike_probabilities = pop['firing_rates'] * (self.timestep_ms / 1000.0)
                    spike_probabilities = cp.minimum(spike_probabilities, 1.0)
    ...
    def simulate_detailed(self, n_steps=100):
        ...
        print(f"\n🧠 Ultra Realistic Purkinje Simulation ({n_steps} steps, {self.timestep_ms}ms timestep):")
...
def test_ultra_realistic_purkinje_simulation():
    """Test ultra realistic Purkinje brain simulation with different timesteps"""
    print("🧠 ULTRA REALISTIC PURKINJE BRAIN SIMULATION")
    print("=" * 80)
    ...
    for timestep in timesteps:
        ...
        try:
            sim = UltraRealisticPurkinjeBrain(timestep_ms=timestep, seed=42)
...
if __name__ == "__main__":
    # Test ultra realistic Purkinje simulation
    test_ultra_realistic_purkinje_simulation()
