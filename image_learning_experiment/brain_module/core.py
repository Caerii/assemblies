...
# Import Area from sibling module
...
class Brain:
  ...
  def project_into(self, target_area, from_stimuli, from_areas, verbose=0):
    ...
    for from_area_name in from_areas:
      ...
      # Check if winners exist before trying to access max
      if from_area.winners.size > 0 and max(from_area.winners) >= connectome.shape[0]:
          raise IndexError(
      ...
      # from_area = area_by_name[from_area_name] # Redundant assignment
    ...
    if target_area.fixed_assembly:
      ...
      num_first_winners_processed = 0
        ...
        # --- START DEBUG PRINT --- 
        # Removed experiment-specific debug: if target_area.name == config.MID_LEVEL: 
        # Removed experiment-specific debug:      print(f"[DEBUG][project_into {target_area.name}] Calculated input sums (shape {all_potential_winner_inputs.shape}): Min={np.min(all_potential_winner_inputs):.2f}, Max={np.max(all_potential_winner_inputs):.2f}, Mean={np.mean(all_potential_winner_inputs):.2f}")
        # --- END DEBUG PRINT --- 
        ...
        # --- START DEBUG PRINT --- 
        # Removed experiment-specific debug: if target_area.name == config.MID_LEVEL:
        # Removed experiment-specific debug:     print(f"[DEBUG][project_into {target_area.name}] Selected top {target_area.k} indices: {new_winner_indices[:10]}...")
        # --- END DEBUG PRINT --- 
