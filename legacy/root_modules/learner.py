...
def lexicon_sizes_experiment(start, end, p=0.05, LEX_k=50, LEX_n=100000, beta=0.06, extra_context_areas=0, extra_context_model="B", use_extra_context=False, extra_context_area_k=20, repeat=1, output_file=None):
    """
    Conducts a lexicon size experiment across a range of noun/verb counts to determine the number of sentences needed to train a brain model.

    Parameters:
    - start, end (int): Range of noun/verb counts to test.
    - p, LEX_k, LEX_n, beta (float): Model parameters.
    - extra_context_areas (int): Number of additional context areas to use.
    - extra_context_model (str): Model type for extra context areas ('B' or 'C').
    - use_extra_context (bool): Whether to use extra context areas in training.
    - extra_context_area_k (int): Size factor for extra context areas.
    - repeat (int): Number of trials to perform for each noun/verb count.
    - output_file (str): File path to write the results to.

    Returns:
    - dict: A dictionary where keys are the noun/verb counts and values are lists of sentences needed for training.
    """
    results = {}
    if output_file:
...
def betas_experiment(start, end, decrement, p=0.05, LEX_k=50, LEX_n=100000, num_nouns=2, num_verbs=2, repeat=1, output_file=None):
    """
    Conducts an experiment to explore the effect of varying beta (learning rate) on the training effectiveness of the brain model.

    Parameters:
    - start, end, decrement (float): Range and step size for beta values to test.
    - p, LEX_k, LEX_n (float): Model parameters.
    - num_nouns, num_verbs (int): Number of nouns and verbs in the model.
    - repeat (int): Number of trials to perform for each beta value.
    - output_file (str): File path to write the results to.

    Returns:
    - dict: A dictionary where keys are the beta values and values are lists of sentences needed for training.
    """
    results = {}
    if output_file:
        f = open(output_file, 'a')
...
def p_experiment(start, end, increment, LEX_k=50, LEX_n=100000, CONTEXTUAL_k=100, PHON_k=100, beta=0.05, num_nouns=2, num_verbs=2):
    """
    Conducts an experiment to explore the effect of varying the parameter 'p' on the training effectiveness of the brain model.

    Parameters:
    - start, end, increment (float): Range and step size for p values to test.
    - LEX_k, LEX_n, CONTEXTUAL_k, PHON_k, beta (float): Model parameters.
    - num_nouns, num_verbs (int): Number of nouns and verbs in the model.

    Returns:
    - dict: A dictionary where keys are the p values and values are the number of sentences needed for training.
    """
    results = {}
    p = start 
    while p <= end:
...
def single_word_tutoring_exp(lex_size_start, lex_size_end, p=0.05, LEX_k=50, LEX_n=100000, beta=0.06, single_word_frequency=2, testing_increment=1,  repeat=1, output_file=None):
    """
    Conducts an experiment to explore the effect of single word tutoring on the training effectiveness of the brain model.

    Parameters:
    - lex_size_start, lex_size_end (int): Range of lexicon sizes to test.
    - p, LEX_k, LEX_n, beta (float): Model parameters.
    - single_word_frequency (int): Frequency of tutoring single words.
    - testing_increment (int): Frequency of testing during training.
    - repeat (int): Number of trials to perform for each lexicon size.
    - output_file (str): File path to write the results to.

    Returns:
    - dict: A dictionary where keys are the lexicon sizes and values are the number of words needed for training.
    """
    results = {}
    if output_file:
        f = open(output_file, 'a')
...
class LearnBrain(brain.Brain):
    """
    Defines a learning brain model that simulates the acquisition and processing of language based on a neural architecture.

    Parameters:
    - p (float): Probability parameter influencing neural connections.
    - PHON_k, CONTEXTUAL_k, EXPLICIT_k, LEX_k (int): Parameters defining the number of features for phonetic, contextual, explicit, and lexical areas, respectively.
    - LEX_n (int): Total number of neurons in the lexical area.
    - beta (float): Learning rate.
    - proj_rounds (int): Number of projection rounds for processing.
    - CORE_k (int): Number of features in the core areas.
    - bilingual (bool): Indicates if the model supports bilingual processing.
    - LANG_k (int): Number of features in the language area.
    - num_nouns, num_verbs (int): Number of nouns and verbs in the model.
    - extra_context_areas (int): Number of additional context areas.
    - extra_context_area_k (int): Size factor for extra context areas.
    - extra_context_model (str): Model type for extra context areas ('B' or 'C').
    - extra_context_delay (int): Delay before activating extra context areas.

    Methods:
    - tutor_single_word(word): Tutors the model with a single word.
    - tutor_single_indexed_word(word_index): Tutors the model with a single word using its index.
    - tutor_random_word(): Tutors the model with a random word.
    - get_extra_context_area_name(index): Retrieves the name of the extra context area based on index.
    - activate_lang(word): Activates the language-specific area based on the word.
    - activate_context(word): Activates the context-specific area based on the word.
    - activate_index_context(word_index, activate_extra_context=False): Activates the context area for the specified word index, with optional activation of extra context.
    - get_context_area(word): Returns the context area based on the word.
    - get_index_context_area(word_index): Returns the context area based on the word index.
    - get_index_lexical_area(word_index): Returns the lexical area based on the word index.
    - activate_PHON(word): Activates the phonetic area based on the word.
    - project_star(mutual_inhibition=False): Executes a projection step, optionally with mutual inhibition.
    - parse_sentence(sentence): Parses and processes a sentence.
    - parse_indexed_sentence(noun_index, verb_index, order="NV"): Parses and processes a sentence based on indices of noun and verb.
    - clear_context_winners(): Clears the winners in context areas.
    - train_simple(rounds): Trains the model with simple predefined sentences.
    - train_random_sentence(): Trains the model with a randomly generated sentence.
    - train_each_sentence(): Trains the model with all combinations of predefined nouns and verbs.
    - train(rounds): Repeatedly trains the model for a number of rounds.
    - train_experiment(max_rounds=100, use_extra_context=False): Trains the model experimentally with a maximum number of rounds, optionally using extra context areas.
    - train_experiment_randomized(max_samples=500, increment=1, start_testing=0, use_extra_context=False): Trains the model experimentally with randomized sentences.
    - train_experiment_randomized_with_tutoring(max_samples=500, testing_increment=1, start_testing=0, single_word_frequency=5): Trains the model experimentally with randomized sentences and tutoring.
    - test_all_words(use_extra_context=False): Tests if all words are correctly recognized by the model.
    - get_explicit_assembly(area_name, min_overlap=0.75): Retrieves the explicit assembly from the specified area.
    - get_PHON(min_overlap=0.75): Retrieves the word associated with the current phonetic assembly.
    - test_verb(word, min_overlap=0.75): Tests the model's ability to recognize a verb.
    - testIndexedWord(word_index, min_overlap=0.75, use_extra_context=False, no_print=False): Tests the model's ability to recognize a word based on its index.
    - test_noun(word, min_overlap=0.75): Tests the model's ability to recognize a noun.
    - get_input_from(from_area, to_area): Calculates the total synaptic input from one area to another.
    - get_total_input(area): Calculates the total synaptic input to the specified area.
    - test_word(word): Tests the model's ability to recognize a word and its properties.
    """
    ...
    def tutor_single_word(self, word):
        """
        Activates the context, phonetic, and language areas (if bilingual) for a single word and projects the neural activity.
        
        Parameters:
        - word (str): The word to tutor.
        """
    ...
    def tutor_single_indexed_word(self, word_index):
        """
        Tutors the model using a single word identified by its index. This function activates the context by index and projects the neural activity.
        
        Parameters:
        - word_index (int): The index of the word to tutor.
        """
    ...
    def tutor_random_word(self):
        """
        Tutors the model using a random word selected from the lexicon. This method selects a word at random by its index and tutors it.
        """
    ...
    def get_extra_context_area_name(self, index):
        """
        Returns the name of the extra context area based on the given index.
        
        Parameters:
        - index (int): The index of the extra context area.
        
        Returns:
        - str: The name of the extra context area.
        """
        return "CONTEXT_" + str(index)

    def activate_lang(self, word):
        """
        Activates the language area based on the given word. This method determines the language index based on whether the word belongs to the primary or secondary language in a bilingual model.
        
        Parameters:
        - word (str): The word whose language context is to be activated.
        """
