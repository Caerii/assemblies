...
_RESULTS: List[Result] = [
    ...
    Result(
        ...
        claim="A transducer whose STATE is its previous arc (state_mode='copy') "
              ...
              "gain (+0.146 full minus blind). MECHANISM UNRESOLVED: the "
              "reported 0.11/0.22 arc contrasts pooled all noninitial positions, "
              "including agreement words that directly encode subject number. "
              "They do not establish distractor-invariant feature carry or "
              "prediction amplifying an existing distractor representation. "
              "See AUDIT_temporal_position_pooling.md. ORDER: two twelve-word "
        ...
        caveat="A prediction result on a synthetic corpus, not a language model: "
               ...
               "TM-9 mechanism verdicts are suspended: the position-pooling "
               "audit invalidates their distractor-specific interpretation, "
               "without changing separately collected MRR observations. A "
               "position-specific rerun is required before claiming a g=0 "
               "representation or g=1 amplification of it. The "
