...
_RESULTS: List[Result] = [
    ...
    Result(
        ...
        caveat="Expansion and quantization are a PAIR. Amplification alone is "
               ...
               "substrate's. WHAT A SOFT SPOT IS (Addenda 4-5, 500 + 300 "
               "organs): the collision of the target block's weakest member "
               "with the area's best-connected outsider -- the outsider a "
               "Binomial(k, p) upper tail (43-50 of 70 rows at p = 0.4), "
               "unrelated to the Cayley graph (intruder 'other' 31/33), the "
               "rate identical across three groups of order 60 and rising "
               "with n_state; the block side is how much of the test-time arc "
               "was potentiated onto the block. PRESENTATIONS HAVE A WINDOW: "
               "8 unformed (95% soft, all words derail), 15 in the window "
               "(0.06%, the two tails just touching), 30 RELOCATED -- the "
               "arc's refraction bias moves the arc past ~20 presentations, "
               "the test-time arc overlaps the trained one by 0.48, and the "
               "soft rate doubles with 12% of words derailing. Training "
               "longer hurts the organ through refraction. STRENGTH IS PINNED "
               "AT BETA (Addendum 6, 300 organs): below beta the cross-context "
               "bias no longer cancels the state-shared neurons' potentiation "
               "and the arc collapses onto the state conjunct (0.05: "
               "across-symbol overlap 0.63, 100% of pairs bad; 0.08: soft rate "
               "1.1%, 18x); above beta a member's net decays and relocates "
               "before 15 presentations. The organ sits at the only working "
               "strength with zero margin. THE SOFT PAIRS ARE REMOVED BY THE GAIN "
               "(Addendum 7): at 20 and 24 presentations, just below the clip "
               "edge c* = ln(w_max max(1, k p) / base) / ln(1 + beta) ~ 28-30, "
               "the organ is EXACT -- 0 soft pairs in 24,000 and 0 derailments "
               "on 100 S5 organs at each; the registered 15 sat where the two "
               "tails just touch. Protocol value: ~0.7-0.85 c*. In-degree "
               "normalisation cut the 15-presentation rate 3.7x by reweighting "
               "the conjuncts (STATE -> ARC in-degree 3,360 vs the stimulus's "
               "16,000), not by the tail. The transducer at half beta is worse "
               "on every count (A3 Amendment 2), so its null is structural. "
               "Apparent group "
               "differences at fixed L were the "
