...
The baseline records historical direct ``json.dump`` sites. Existing studies
must be migrated deliberately; a new site is an accidental bypass and fails
the maintained test gate.
"""
...
def test_direct_result_writes_do_not_grow_without_disposition():
    baseline = json.loads(BASELINE.read_text(encoding="utf-8"))["direct_json_dump_sites"]
