...
def _extended_metrics(params: dict) -> dict:
    ...
    # One owner for the source-training schedule. A Brain-level recurrence
    # default cannot override ops.project's explicit operation contract.
    from neural_assemblies.programs.pnas_extended import run_pnas_reciprocal
    recip_ov = run_pnas_reciprocal(
        seed=seed, n=n, k=k, p=p_conn, beta=beta, rounds=rounds,
        recurrent=True,
