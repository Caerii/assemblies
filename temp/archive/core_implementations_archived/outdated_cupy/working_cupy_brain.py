...
"""
Working CuPy Brain
Bypasses CURAND initialization issues for GPU acceleration
"""
...
try:
    ...
    # Test CuPy without random number generation
    try:
        test_array = cp.zeros(1000, dtype=cp.float32)
        print("✅ CuPy basic operations working!")
        CUPY_AVAILABLE = True
    except Exception as e:
        print(f"❌ CuPy basic operations failed: {e}")
...
class WorkingCuPyBrain:
    ...
    def __init__(self, n_neurons=1000000000, active_percentage=0.0001, n_areas=5, seed=42):
        """Initialize the working CuPy brain"""
        ...
        # Initialize random number generator (CPU-based to avoid CURAND issues)
        self._rng = np.random.default_rng(seed)
        
        print(f"🌍 Working CuPy Brain initialized:")
        ...
        if use_gpu:
            print(f"   Note: Using CPU random generation + GPU computation (CURAND bypass)")
    ...
    def _generate_candidates_optimized(self, area_idx):
        ...
        if self.use_gpu:
            # Use CPU random generation + GPU computation (CURAND bypass)
...
def test_working_cupy_brain():
    """Test working CuPy brain"""
    print("🌍 TESTING WORKING CUPY BRAIN")
    print("=" * 60)
    ...
    # Summary
    print(f"\n📊 WORKING CUPY BRAIN BENCHMARK SUMMARY")
    print("=" * 80)
...
if __name__ == "__main__":
    # Test working CuPy brain
    results = test_working_cupy_brain()
