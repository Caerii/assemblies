...
- 2026-09-12: Emergent Nemo brain fully passes focused Pyright (0 errors) after replacing untyped CuPy array annotations, narrowing optional activation state before reads, and routing Torch top-k/as_tensor through the shared boundary.
...
- 2026-09-12: Source-only `nemo/language` audit (46 files excluding tests/archive): 0 errors, 0 warnings, 0 informational diagnostics under Pyright.
- 2026-09-12: Core shipped source gate (`assembly_calculus`, `core`, `compute`, 196 files): 0 errors, 0 warnings, 0 informational diagnostics under Pyright.
...
- 2026-09-12: Shared experiment runner exposes `--list`, a registry-only discovery command that avoids importing study modules or GPU extensions; registry test passed (2 tests), runner Pyright 0 errors.
- 2026-09-12: Operation contract registry validated end-to-end: 163 contract tests passed; all registered operations expose specification links, failure conditions, and constructed true-negative controls. Specification-link audit completed with exit 0 and no errors.
- 2026-09-12: Migrated `seq_tm_high_order.py` onto the shared immutable runner as `temporal-memory-high-order`; GPU implementation is imported only after parser/reservation, hashed transducer semantics are recorded for each arm, and the legacy result writer/sys.path path is removed. Registry tests: 2 passed; focused Pyright: 0 errors.
- 2026-09-12: Migrated `seq_a1_fsm_parity.py` to the shared runner as `a1-fsm-parity`; removed bespoke output/sys.path setup, made parser failure occur before study execution, and preserved explicit main/null/degen/constant-refraction arms. Registry tests: 2 passed; focused Pyright: 0 errors.
- 2026-09-12: Added accelerator-import ratchet for all 15 registered experiments: top-level torch/CuPy imports are rejected so parser/engine/tag validation precedes extension loading. Registry suite: 3 passed.
- 2026-09-12: Migrated the preregistered S5 soft census to `s5-soft-census` on the shared runner. Removed module-level GPU imports and sys.path mutation; hashed-FSM semantics, groups, presentations, and null-sensitive parameters are recorded before measurement. Import smoke confirms torch is not loaded; registry tests: 3 passed; focused Pyright: 0 errors.
- 2026-09-12: Migrated the preregistered A1 exactness sweep to `a1-exactness-sweep`; materialized versus sampled substrate is an explicit parameter, not an environment side channel, and the old direct result writer/sys.path path is removed. Import smoke loads no Torch; registry tests: 3 passed; focused Pyright: 0 errors.
- 2026-09-12: Migrated preregistered `seq_a1_local_regime.py` to `a1-local-regime`; local organ density, ambient traffic, presentations, and materialization are explicit runner parameters. Removed environment/path/result-writer side channels. Import smoke loads no Torch; registry tests: 3 passed; focused Pyright: 0 errors.
- 2026-09-12: Migrated preregistered `seq_a2_refraction_load.py` to `a2-refraction-load`; arc sizes, materialization, presentations, and ambient/organ densities are explicit runner parameters. Removed environment/path/result-writer side channels. Import smoke loads no Torch; registry tests: 3 passed; focused Pyright: 0 errors.
- 2026-09-12: Documented the migration boundary for unregistered S5 diagnostics: legacy tiered paths remain forensic-only until backend/materialization semantics are explicit, preventing accidental scientific registration.
...
## 2026-09-12 ? absence curriculum randomness
...
## 2026-09-12 sequence value boundary
...
## 2026-09-12 atomic Brain IR program admission
