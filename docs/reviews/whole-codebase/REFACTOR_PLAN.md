...
|---|---|---|
| Runtime configuration | Immutable primary-path model identity across graph/candidates/stimulus/ties/arithmetic/normalization/plasticity; immutable homeostasis and competition policies; explicit sampled-recurrence admission; explicit area/stimulus identity; shared numeric and slot validation; executing-owner LRI, refraction, policy and noise controls | Add area-local inhibition/refraction and operation schedule identities to complete protocol documents; remaining topology controls; general registration transactionality |
...
| Configuration transport | Protocol, homeostasis and competition schemas share Python/Rust acceptance corpora; schema-6 Brain runs require and reconstruct their canonical model profile; runner fixtures reconstruct recorded configuration | Hashed-organ semantics; production experiment adoption; toolchain/executable identities and legacy document migration |
