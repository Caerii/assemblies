...
`associate_trace` now uses `AssociationPlan` for schedule, clamping, and preflight, eliminating duplicated phase logic. Trace labels follow the actual plan steps; focused trace and operation-contract tests pass (163 tests).
...
Convergence learning is now registered as a first-class operation contract. Both learn_assembly and learn_assembly_from_pattern attach to the shared frozen ConvergencePlan and contract-convergence semantic card, and both registry names resolve to that contract. The existing invalid-schedule controls remain green (181 focused contract/spec tests).

A naming ambiguity is now explicit in the contract graph: ops.bind and assembly_calculus.binding.bind are different operations with different inputs, schedules, and return values. The parser-facing multi-source function is represented by SourceBindingPlan/SOURCE_BINDING_CONTRACT and contract-source-binding, while the package-level bind retains the single-source snapshot contract. Both are decorator-checked and publicly inspectable.

The parser-facing binding pair is now explicit in the IR-facing contract graph: SourceBindingPlan/SOURCE_BINDING_CONTRACT for multi-source teacher binding and BindingRecallPlan/BINDING_RECALL_CONTRACT for its readout. This distinguishes them from ops.bind and read_binding, which have different schedules and return values. Both plans validate before mutation or observation, and registry/decorator/source-link ratchets cover them.
