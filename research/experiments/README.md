...
| sequence | `seq_a1_exactness_sweep.py` | exact state recovery versus afferent probability, with sampled/materialized substrate control | `python -m research.runner a1-exactness-sweep --tag UNIQUE --materialized` |
| sequence | `seq_a1_local_regime.py` | local organ density embedded in an ambient sparse brain, with materialization control | `python -m research.runner a1-local-regime --tag UNIQUE --materialized` |
| sequence | `seq_a2_refraction_load.py` | refraction convergence across arc load for single- and multi-mood FSMs | `python -m research.runner a2-refraction-load --tag UNIQUE --materialized` |
...
| sequence | `seq_s5_soft_census_hashed.py` | hashed assigned-state census, conjunction overlap, and first-deviation mechanism | `python -m research.runner s5-soft-census --tag UNIQUE` (10 seeds by default); add `--smoke --seeds 42 43 44` only for a VOID API check |
