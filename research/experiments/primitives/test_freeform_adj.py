"""
Free-Form Learner with Adjectives: 4-Category Discovery

Tests whether adding adjectives to the DET grammar enables the FreeFormLearner
to discover 4 lexical categories (DET, ADJ, NOUN, VERB) from raw sentence
exposure. This directly tests whether the 2→3 category scaling (determiners)
generalizes to 2→3→4.

The DET-ADJ-N-V-DET-ADJ-N pattern creates a 4-position cycle that provides
distributional evidence for a 4-way split. With 4 CORE areas and
refractory_period=3, the routing mechanism cycles through all four areas:
  Position 0 (DET)  → CORE_0
  Position 1 (ADJ)  → CORE_1
  Position 2 (NOUN) → CORE_2
  Position 3 (VERB) → CORE_3
  Position 4 (DET)  → CORE_0 (cycle repeats)

Each CORE area trains a specific prediction connectome:
  CORE_0 (DET)  → PREDICTION: predicts adjectives only
  CORE_1 (ADJ)  → PREDICTION: predicts nouns only
  CORE_2 (NOUN) → PREDICTION: predicts verbs (or DET at sentence boundary)
  CORE_3 (VERB) → PREDICTION: predicts determiners only
...
Definitive results (5 seeds, n=10000, 400 train sentences, 30 test each):
  H1: PASS  stability = 0.816 +/- 0.004
  H2: PASS  N400 d = 1.86 +/- 0.18   (1.55x improvement over 3-CORE)
  H3: PASS  binding = 100.0% +/- 0.0%
  H4: FAIL  2/5 seeds improved (prediction already near-ceiling from start)
  H5: PASS  novel d = 1.75 +/- 0.13   (1.65x improvement over 3-CORE)

  Category discovery: 23/23 correct across ALL 5 seeds.
  Identical split every seed: DET(2)->CORE_0, ADJ(5)->CORE_1,
                              NOUN+LOC(8)->CORE_2, VERB+PREP(8)->CORE_3

Usage:
    uv run python research/experiments/primitives/test_freeform_adj.py
    uv run python research/experiments/primitives/test_freeform_adj.py --quick
"""
...
class AdjExperimentConfig:
    ...
    n_struct_areas: int = 12     # longer sentences (7-11 words)
...
def make_learner_config(cfg: AdjExperimentConfig) -> FreeFormConfig:
...
def generate_novel_adj_combinations(
    vocab, rng: np.random.Generator, n_items: int,
) -> List[List[str]]:
    """Generate DET-ADJ-N-V-DET-ADJ-N sentences with novel word combinations."""
    ...
    for _ in range(n_items):
        ...
        adj1 = adjs[rng.integers(len(adjs))]
        adj2 = adjs[rng.integers(len(adjs))]
        sent = [det1, adj1, agent, verb, det2, adj2, patient]
        # ~30% chance of PP
        if rng.random() < 0.3:
            prep = preps[rng.integers(len(preps))]
            loc = locs[rng.integers(len(locs))]
            det3 = dets[rng.integers(len(dets))]
            adj3 = adjs[rng.integers(len(adjs))]
            sent.extend([prep, det3, adj3, loc])
        combos.append(sent)
    return combos


def run_trial(cfg: AdjExperimentConfig, seed: int) -> Dict[str, Any]:
    """Run one trial of free-form learning with adjectives."""
    ...
    # Generate curriculum with adjectives (only words, no labels)
    grammar = AdjCFG(pp_prob=0.3, vocab=vocab, rng=rng)
    curriculum = grammar.generate_batch(cfg.n_train_sentences)

    # Generate test sets
    test_grammar = AdjCFG(pp_prob=0.3, vocab=vocab,
    ...
    novel_sents = generate_novel_adj_combinations(
...
class FreeFormAdjExperiment(ExperimentBase):
    """Free-form learner with adjectives experiment."""
    ...
    def run(
        self,
        n_seeds: int = 5,
        config: Optional[AdjExperimentConfig] = None,
        **kwargs,
    ) -> ExperimentResult:
        self._start_timer()

        cfg = config or AdjExperimentConfig()

        self.log("=" * 70)
        self.log("Free-Form Learner + Adjectives: 4-Category Discovery")
...
def main():
    parser = argparse.ArgumentParser(
        description="Free-Form Learner + Adjectives Experiment")
    parser.add_argument("--quick", action="store_true")
    parser.add_argument("--seeds", type=int, default=None)
    args = parser.parse_args()

    exp = FreeFormAdjExperiment(verbose=True)

    if args.quick:
        cfg = AdjExperimentConfig(
            n=5000, k=50,
    ...
    print("\n" + "=" * 70)
    print("FREE-FORM LEARNER + ADJECTIVES SUMMARY")
    print("=" * 70)
