...
Measurement:
    A custom _measure_agreement_word() function extends _measure_critical_word()
...
from research.experiments.metrics.instability import (
    compute_jaccard_instability, measure_p600_settling,
)
...
def _measure_agreement_word(
    ...
    Like _measure_critical_word() from test_p600_syntactic.py, but also
    projects number stimuli into the NUMBER area during context processing.

    Key difference from previous version: at verb position, we fix the
    subject's NOUN_CORE assembly alongside VERB_CORE and NUMBER during
    settling. This matches the consolidation pattern:
      - VP consolidation: NOUN_CORE + VERB_CORE + NUMBER -> VP
      - ROLE consolidation: NOUN_CORE + NUMBER -> ROLE

    Without fixing NOUN_CORE, the VP settling only receives VERB_CORE +
    NUMBER, which is incomplete—the full three-way agreement pattern
    ...
    # The Jaccard instability metric captures random-vs-consolidated signal
    # quality, not competition between mismatched consolidated patterns.
    # Two additional metrics:
    #
    # 1. Winner margin: gap between k-th winner and (k+1)-th loser in
    #    pre-k-WTA activations. Small margin = high competition = hard
    #    integration. (Hagoort 2005: unification difficulty)
    #
    # 2. VP assembly: the actual winner set from projecting into VP.
    #    By comparing VP assemblies between conditions (gram vs agree),
    #    we directly measure how much the structural representation
    #    changes due to agreement violation. (Brouwer & Crocker 2017:
    #    integration update cost)
    vp_margin = 0.0
    vp_winners = set()
    if VP in p600_areas:
    ...
    def run(self, quick=False, **kwargs):
        ...
        for seed_idx, seed in enumerate(seeds):
            ...
            for test in VERB_TESTS:
                ...
                result_gram = _measure_agreement_word(
                ...
                result_agree = _measure_agreement_word(
            ...
            for test in OBJECT_TESTS:
                ...
                result_cat = _measure_agreement_word(
