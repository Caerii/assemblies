...
def memory_recall_vs_M():
    ...
    ax.set_ylabel("fraction of stored items returned from a half cue")
...
def memory_ceiling_vs_nk():
    ...
    ax.set_xlabel("n / k")
    ax.set_ylabel("capacity M*: stored items at which recall crosses 1/2")
...
def organ_soft_rate():
    ...
    for ax, series, xlabel, title in (
        ...
        ax.set_ylabel("soft transitions, % of all (state, symbol) pairs", color="#1f77b4")
        ax2 = ax.twinx()
...
def organ_arc_drift():
    ...
    ax.set_ylabel("overlap of that presentation's arc with the test arc (fraction of k)")
...
def organ_order10_window():
    ...
    ax.set_ylabel("fraction of 20 brains predicting the right continuation")
...
def memory_gate_U():
    """Three stacked panels against stored items, with the convergence gate
    on: recall; the fraction of items whose write converged within the
    budget; the rounds each write took."""
    try:
    ...
    axes[0].plot(ms, rank1, "-o", ms=3, color="#d62728")
    axes[0].axhline(0.5, color="k", lw=0.6, ls=":")
    axes[0].set_title("recall: fraction of stored items that come back from a half cue", loc="left", fontsize=9)
    axes[0].set_ylabel("fraction")
    axes[0].set_ylim(-0.03, 1.05)
    axes[1].plot(ms, conv, "-s", ms=3, color="#1f77b4")
    axes[1].set_title("fraction of items whose winners settled within the 8-round write", loc="left", fontsize=9)
    axes[1].set_ylabel("fraction")
    axes[1].set_ylim(-0.03, 1.05)
    axes[2].plot(ms, rounds, "-^", ms=3, color="#555555")
    axes[2].set_title("rounds each write took before its winners settled (8 = never)", loc="left", fontsize=9)
    axes[2].set_ylabel("rounds")
    axes[2].set_ylim(4, 8.5)
    axes[2].set_xscale("log", base=2)
    axes[2].set_xlabel("stored assemblies M (n = 4000, k = 60, refracted, gated write)")
    fig.suptitle("recall fails where items stop settling within the write budget", fontsize=10)
    fig.tight_layout()
...
def throughput():
    ...
    ax.set_ylabel("throughput per second (log scale)")
